...
{
    public partial class AreaProcessual : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Master.litCaminhoSecundario.Text = "Área Processual";
        }
        ...
        {
            grdResultado.Sort("idAreaProcessual", SortDirection.Ascending);
        }
        ...
        protected void btnExcluirSelecionados_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in grdResultado.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    HiddenField hdIdAreaProcessual = (HiddenField)row.FindControl("hdIdAreaProcessual");

                    dtoAreaProcessual areaProcessual = bllAreaProcessual.Get(Convert.ToInt32(hdIdAreaProcessual.Value));

                    if (chkExcluir.Checked && areaProcessual != null)
                        bllAreaProcessual.Delete(Convert.ToInt32(areaProcessual.idAreaProcessual));
                }
            }
        ...
        {
            litTotalRegistros.Text = String.Format("{0} registro(s) encontrado(s)", ((List<dtoAreaProcessual>)e.ReturnValue).Count.ToString());
        }
        ...
        protected void grdResultado_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                string idAreaProcessual = DataBinder.Eval(e.Row.DataItem, "idAreaProcessual").ToString();
                string Location = ProJur.DataAccess.Configuracao.getEnderecoVirtualSite() + "/Paginas/Manutencao/AreaProcessual.aspx?ID=" + idAreaProcessual;
        ...
        {
            List<dtoAreaProcessual> listaItems = bllAreaProcessual.GetAll("Descricao", prefixText);
            List<string> listaRetorno = new List<string>();

            foreach (dtoAreaProcessual item in listaItems)
            {
