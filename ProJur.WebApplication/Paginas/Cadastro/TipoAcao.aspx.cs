...
{
    public partial class TipoAcao : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Master.litCaminhoSecundario.Text = "Tipo de Ação";
        }
        ...
        {
            grdResultado.Sort("idTipoAcao", SortDirection.Ascending);
        }
        ...
        protected void btnExcluirSelecionados_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in grdResultado.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    HiddenField hdIdTipoAcao = (HiddenField)row.FindControl("hdIdTipoAcao");

                    dtoTipoAcao TipoAcao = bllTipoAcao.Get(Convert.ToInt32(hdIdTipoAcao.Value));

                    if (chkExcluir.Checked && TipoAcao != null)
                        bllTipoAcao.Delete(Convert.ToInt32(TipoAcao.idTipoAcao));
                }
            }
        ...
        {
            litTotalRegistros.Text = String.Format("{0} registro(s) encontrado(s)", ((List<dtoTipoAcao>)e.ReturnValue).Count.ToString());
        }
        ...
        protected void grdResultado_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                string idTipoAcao = DataBinder.Eval(e.Row.DataItem, "idTipoAcao").ToString();
                string Location = ProJur.DataAccess.Configuracao.getEnderecoVirtualSite() + "/Paginas/Manutencao/TipoAcao.aspx?ID=" + idTipoAcao;
        ...
        {
            List<dtoTipoAcao> listaItems = bllTipoAcao.GetAll("Descricao", prefixText);
            List<string> listaRetorno = new List<string>();

            foreach (dtoTipoAcao item in listaItems)
            {
